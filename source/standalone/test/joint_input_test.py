...
from omni.isaac.lab.sensors import CameraCfg, ContactSensorCfg, Camera, ContactSensor
...
f = open('/home/jmin/catkin_ws/1.csv', 'r')
reader = csv.reader(f)
data = list(reader)
...
def define_sensor():
    ...
    return ray_caster, camera, contact_sensor
...
def design_scene() -> tuple[dict, list[list[float]]]:
    ...
    ray_caster, camera, contact_sensor = define_sensor()

    # return the scene information
    scene_entities = {"hamster_n": hamster_n, "ray_caster": ray_caster, "camera": camera, "contact_sensor": contact_sensor}
...
def run_simulator(sim: sim_utils.SimulationContext, entities: dict[str, Articulation], origins: torch.Tensor):
    ...
    while simulation_app.is_running():
        for i in data:
            test_pos = torch.tensor(list(map(float, i[0:12])), dtype=torch.float)
            test_vel = torch.tensor(list(map(float, i[12:24])), dtype=torch.float)
            test_effort = torch.tensor(list(map(float, i[24:36])), dtype=torch.float)
